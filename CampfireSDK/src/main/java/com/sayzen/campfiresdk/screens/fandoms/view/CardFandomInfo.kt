...
class CardFandomInfo(
    ...
    private val eventBus = EventBus
        ...
        .subscribe(EventFandomKarmaCofChanged::class) {
            if (it.fandomId == xFandom.getId()) karmaCof = it.cof; updateKarmaCof();
        }
        ...
        .subscribe(EventFandomCategoryChanged::class) {
            if (it.fandomId == xFandom.getId()) categoryId = it.newCategory
        }
    ...
    private fun setCof() {
        SplashFieldTwo()
            .setTitle(
                t(
                    API_TRANSLATE.fandoms_menu_set_cof_hint,
                    API.FANDOM_KARMA_COF_MIN / 100,
                    API.FANDOM_KARMA_COF_MAX / 100
                )
            )
    ...
    fun updateParams_1() {
        val view = getView() ?: return
        view.findViewById<View>(R.id.vParams1_Root).visibility =
            if (CampfireConstants.getParamTitle(categoryId, 1) != null) View.VISIBLE else View.GONE
        ...
        vEmptyText.text = if (params1.isEmpty() && CampfireConstants.getParamTitle(
                categoryId,
                1
            ) != null
        ) CampfireConstants.getParamTitle(categoryId, 1)!! else ""
    ...
    fun updateParams_2() {
        val view = getView() ?: return
        view.findViewById<View>(R.id.vParams2_Root).visibility =
            if (CampfireConstants.getParamTitle(categoryId, 2) != null) View.VISIBLE else View.GONE
        ...
        vEmptyText.text = if (params2.isEmpty() && CampfireConstants.getParamTitle(
                categoryId,
                2
            ) != null
        ) CampfireConstants.getParamTitle(categoryId, 2)!! else ""
    ...
    fun updateParams_3() {
        val view = getView() ?: return
        view.findViewById<View>(R.id.vParams3_Root).visibility =
            if (CampfireConstants.getParamTitle(categoryId, 3) != null) View.VISIBLE else View.GONE
        ...
        vEmptyText.text = if (params3.isEmpty() && CampfireConstants.getParamTitle(
                categoryId,
                3
            ) != null
        ) CampfireConstants.getParamTitle(categoryId, 3)!! else ""
    ...
    fun updateParams_4() {
        val view = getView() ?: return
        view.findViewById<View>(R.id.vParams4_Root).visibility =
            if (CampfireConstants.getParamTitle(categoryId, 4) != null) View.VISIBLE else View.GONE
        ...
        vEmptyText.text = if (params4.isEmpty() && CampfireConstants.getParamTitle(
                categoryId,
                4
            ) != null
        ) CampfireConstants.getParamTitle(categoryId, 4)!! else ""
    ...
    private fun change(paramsPosition: Int, params: Array<Long>) {
        val paramTitle = CampfireConstants.getParamTitle(categoryId, paramsPosition)
        if (paramTitle == null) {
        ...
        SplashParams(
            paramTitle,
            CampfireConstants.getParams(categoryId, paramsPosition)!!,
            params
        ) { newParams, comment ->
            ApiRequestsSupporter.executeEnabledConfirm(
                t(API_TRANSLATE.fandoms_menu_change_params_confirms),
                t(API_TRANSLATE.app_change),
                RFandomsAdminChangeParams(xFandom.getId(), categoryId, paramsPosition, newParams, comment)
            ) {
                EventBus.post(
                    EventFandomParamsChanged(
                        xFandom.getId(),
    ...
    fun updateDescription() {
        ...
        vDescriptionChange.visibility = if (ControllerApi.can(
                xFandom.getId(),
                xFandom.getLanguageId(),
                API.LVL_MODERATOR_DESCRIPTION
            )
        ...
        if (ControllerApi.can(
                xFandom.getId(),
                xFandom.getLanguageId(),
                API.LVL_MODERATOR_NAMES
            )
        ) vNames.setOnClickListener { SplashNames(xFandom.getId(), xFandom.getLanguageId(), names).asSheetShow() }
    }

    private fun changeDescription() {
        SplashDescription(description) { description ->
            ControllerApi.moderation(
                t(API_TRANSLATE.app_moderation),
                t(API_TRANSLATE.app_confirm),
                { RFandomsModerationDescriptionChange(xFandom.getId(), xFandom.getLanguageId(), description, it) }) {
    ...
    fun updateGallery() {
        ...
        vAdd.visibility = if (ControllerApi.can(
                xFandom.getId(),
                xFandom.getLanguageId(),
                API.LVL_MODERATOR_FANDOM_IMAGE
            )
                ...
                Navigator.to(SCrop(b) { _, bitmap, _, _, _, _ ->
                    SplashField()
                        ...
                        .setOnEnter(t(API_TRANSLATE.app_add)) { _, comment ->
                            ...
                            ToolsThreads.thread {
                                val image = ToolsBitmap.toBytes(
                                    ToolsBitmap.keepMaxSides(bitmap, API.FANDOM_GALLERY_MAX_SIDE),
                                    API.FANDOM_GALLERY_MAX_WEIGHT
                                )
                                ToolsThreads.main {
        ...
        for (link in links) {
            ...
            val w = SplashMenu()
                .add(t(API_TRANSLATE.app_copy_link)) {
                    ToolsAndroid.setToClipboard(link.url);ToolsToast.show(
                    t(
                ...
                .add(t(API_TRANSLATE.app_remove)) { removeLink(link) }
                .condition(ControllerApi.can(xFandom.getId(), xFandom.getLanguageId(), API.LVL_MODERATOR_LINKS))
    ...
    private fun addLink() {
        ...
        SplashLink("", "", 0) { w, url, title, comment, imageIndex ->
            ApiRequestsSupporter.executeEnabled(
                w,
                RFandomsModerationLinkAdd(xFandom.getId(), xFandom.getLanguageId(), title, url, imageIndex, comment)
            ) { r ->
