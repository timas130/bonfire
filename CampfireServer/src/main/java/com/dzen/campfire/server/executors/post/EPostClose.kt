...
class EPostClose : RPostClose(0) {
    ...
    override fun execute(): Response {
        ...
        Database.update("EPostClose", SqlQueryUpdate(TPublications.NAME).where(TPublications.id, "=", publicationId).update(TPublications.closed, 1))
        ControllerPublicationsHistory.put(publicationId, HistoryClose(apiAccount.id, apiAccount.imageId, apiAccount.name))
