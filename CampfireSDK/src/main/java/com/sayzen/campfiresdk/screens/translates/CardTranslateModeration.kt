...
class CardTranslateModeration(
        val history:TranslateHistory
) : Card(R.layout.screen_translates_card_moderation) {

    val eventBus = EventBus.subscribe(EventTranslateHistoryRemove::class) { if(it.id == history.id) adapter.remove(this)}
    ...
    override fun bindView(view: View) {
        ...
        view.setOnClickListener {
            Navigator.to(STranslateMake(history.key, history.languageId, history.fromLanguageId, history, history.type == TranslateHistory.TYPE_HINT))
        }
        ...
        if (history.oldText.isEmpty()) {
            ...
            if (history.type == TranslateHistory.TYPE_TEXT) {
                vText.text = tCap(API_TRANSLATE.translates_label_history_card_translate,
                        tSex(xAccount.getSex(), API_TRANSLATE.he_added, API_TRANSLATE.she_added),
                        history.newText
                )
            } else {
                vText.text = tCap(API_TRANSLATE.translates_label_history_card_hint,
                        tSex(xAccount.getSex(), API_TRANSLATE.he_added, API_TRANSLATE.she_added),
                        history.newText
                )
            }
