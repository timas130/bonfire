...
use crate::mechanics::daily_task::task::determine_task;
...
/*
create table public.random_seeds (
    date date not null primary key,
    seed bytea not null
);

create table public.daily_tasks (
    account_id bigint not null references campfire_db.accounts on delete cascade,
    date date not null,
    json_db jsonb not null,
    progress float not null default 0,
    combo_multiplier float not null default 1,
    level_multiplier float not null default 0,
...
pub async fn get_dt_level(
    ...
    if !today_task_generated {
        ...
        let task = determine_task(context, user_id, level, &today).await?;
        ...
        let combo_multiplier = match task_history.last() {
            Some(task) => get_new_combo_multiplier(task.combo_multiplier, task.progress),
            None => 1.,
        };
