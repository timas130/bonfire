...
private fun JettyConfig.createHttp2Server(jksPath: String, jksPassword: String, portV1: Int, portV2: Int) {
    ...
    addConnector { server, _ ->
        ServerConnector(server).apply { port = portV1 }
    }
    addConnector { server, _ ->
        ServerConnector(server, ssl, alpn, http2, fallback).apply { port = portV2 }
    }
}
    ...
    Javalin
        .create { config ->
            ...
            config.routes.post("/") { ctx ->
                handleCf2Request(ctx)
            }
            config.jetty.createHttp2Server(jksPath, jksPassword, portV1, portV2)
        }
        .start()
}

private fun ApiServer.handleCf2Request(ctx: Context) {
    ctx.with(RateLimitPlugin::class).requestPerTimeUnit(300, TimeUnit.MINUTES)
    ...
    try {
        ...
        return
    } catch (_: ApiServer.TooManyRequestsException) {
