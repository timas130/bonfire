...
class CardChat(
    ...
    init {
        ...
        xFandom = XFandom().setId(if (chat.tag.chatType == API.CHAT_TYPE_FANDOM_ROOT) chat.tag.targetId else 0).setLanguageId(chat.tag.targetSubId).setName(chat.customName).setImage(chat.customImage).setOnChanged { update() }
