...
class EProjectGetEffect : RProjectGetEffect(0) {
    ...
    override fun execute(): Response {
        ...
        if (effectId == API.EFFECT_INDEX_PIG) {
            ControllerEffects.makeSystem(ControllerAccounts.getAccount(apiAccount.id)!!, API.EFFECT_INDEX_PIG, System.currentTimeMillis()+1000L*60*60*24, API.EFFECT_COMMENT_UNCULTURED)
        }
