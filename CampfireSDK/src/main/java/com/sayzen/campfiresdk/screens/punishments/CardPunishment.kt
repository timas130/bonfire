...
class CardPunishment(
    ...
    init {
        ...
        if (punishment.fandomId == 0L) {
            if (punishment.banDate > 0) text = tCap(
                API_TRANSLATE.profile_punishment_card_ban_admin,
                ControllerLinks.linkToAccount(punishment.fromAccountName),
                tSex(
                    CampfireConstants.RED,
                    punishment.fromAccountSex,
            ...
            if (punishment.banDate > 0) text = tCap(
                API_TRANSLATE.profile_punishment_card_ban,
                ControllerLinks.linkToAccount(punishment.fromAccountName),
                tSex(
                    CampfireConstants.RED,
                    punishment.fromAccountSex,
                ...
                ControllerLinks.linkToAccount(punishment.fromAccountName),
                tSex(
                    CampfireConstants.YELLOW,
                    punishment.fromAccountSex,
                    API_TRANSLATE.he_warn,
                    API_TRANSLATE.she_warn
                ),
                "" + punishment.fandomName
            )
        ...
        if ((ControllerApi.isCurrentAccount(punishment.fromAccountId) || ControllerApi.can(API.LVL_ADMIN_USER_PUNISHMENTS_REMOVE))
            ...
            setOnLongClick { _, view, _, _ ->
                SplashMenu()
                    .add(t(API_TRANSLATE.app_remove)) { removePunishment() }.backgroundRes(R.color.red_700)
        ...
        if (punishment.fandomId > 0) setOnCLickAvatar {
            SFandom.instance(
                punishment.fandomId,
                punishment.languageId,
                Navigator.TO
            )
        }
    ...
    override fun onBind(vAvatar: ViewAvatarTitle) {
        ...
        if (punishment.fandomAvatar.isNotEmpty()) ImageLoader.load(punishment.fandomAvatar)
            .into(vAvatar.vAvatar.vImageView)
        else ImageLoader.load(punishment.fromAccountAvatar).into(vAvatar.vAvatar.vImageView)

        if (punishment.fandomId != 0L) {
            ToolsView.addLink(vAvatar.vTitle, punishment.fandomName) {
                SFandom.instance(
                    punishment.fandomId,
                    punishment.languageId,
                    Navigator.TO
    ...
    override fun notifyItem() {
        if (punishment.fandomAvatar.isNotEmpty()) ImageLoader.load(punishment.fandomAvatar).intoCash()
        else ImageLoader.load(punishment.fromAccountAvatar).intoCash()
    }
    ...
    private fun removePunishment() {
        ControllerApi.moderation(
            t(API_TRANSLATE.profile_remove_punishment),
            t(API_TRANSLATE.app_remove),
            { RAccountsAdminPunishmentsRemove(punishment.id, it) }) {
            ToolsToast.show(t(API_TRANSLATE.app_done))
            EventBus.post(
                EventAccountPunishmentRemove(
                    punishment.id,
                    punishment.ownerId,
                    punishment.banDate > 0,
                    punishment.banDate <= 0
                )
            )
